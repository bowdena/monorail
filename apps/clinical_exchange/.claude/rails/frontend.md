...
Aim for progressive enhancement, prefer turbo frames over turbo streams over 
broadcasts.  Always aim for the simplest implementation that delivers the outcome
required unless otherwise asked.
...
**Strict locals** - declare every partial's signature on the first line:
```erb
<%# locals: (items:, id: "accordion", multiple: false, classes: nil) %>
```

- The `locals:` magic comment is the partial's **first content**, before
  any descriptive doc comment; it may wrap across lines for a long
  signature. Only one is allowed per partial.
- Locals **without** a default (`items:`) are required — render raises
  `ActionView::StrictLocalsError` if a caller omits them.
- Locals **with** a default carry it in the signature; drop the
  `local_assigns.fetch(:x, default)` boilerplate from the body.
- Reference locals as plain variables (`classes`), not `local_assigns[:classes]`.
- Passing any local not in the signature raises — a deliberate guardrail.
  Check callers when adding strict locals to an existing partial.
